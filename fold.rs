...
pub proof fn lemma_fold_left_preserves_inv<A, B>
    ...
    requires
        inv(v),
        forall |e:A, v:B| inv(v) ==> #[trigger]inv(f(v, e)),
    ensures
        inv(l.fold_left(v, f))
    decreases l.len(),
{
    ...
    decreases s.len(),
{
    if s.len() > 0 {
        ...
        // assert forall |x:A| #[trigger]s.drop_last().contains(x) implies inv_e(x) by{
        //     assert(s.contains(x))
        // }

        assert(inv_e(s.last())) by { assert(s.contains(s.last())) };
        // assert(inv(f(s.last(), v)));

        lemma_fold_right_commute_one_with_inv(s.drop_last(), a, f, f(s.last(), v), inv, inv_e);

        // assert(s.drop_last().fold_right(f, f(a, f(s.last(), v))) ==
        //     f(a, s.drop_last().fold_right(f, f(s.last(), v))));

        // assert(f(a, f(s.last(), v)) == f(s.last(), f(a, v))) by {
        //     // assert(inv_e(a));
        //     // assert(inv(v));
        // }
    }
